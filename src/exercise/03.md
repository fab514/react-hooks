...
Only use lifting state when multiple siblings need access to the same state. If
the siblings are not needed the colocate state moves the state to the local
