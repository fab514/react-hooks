...
callback function which React will call after the DOM has been updated: A
...
Lazy Initialization - If you pass a function to useState, React will only call
the function when it needs the initial value (which is when the component is
initially rendered).
